...
namespace NLoptNet
{
    ...
    public partial class NLoptSolver : IDisposable
    {
        ...
        private delegate double nlopt_func(
            uint n,
            [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 0), In]
            double[] x,
        ...
        public NLoptSolver(NLoptAlgorithm algorithm, uint numVariables, double relativeStoppingTolerance = 0.0001,
            int maximumIterations = 0, NLoptAlgorithm? childAlgorithm = null)
        {
            if (numVariables < 1)
            ...
            if (relativeStoppingTolerance > 0.0)
            {
                ...
                if (res != NloptResult.SUCCESS)
                    throw new ArgumentException("Unable to set primary tolerance. Result: " + res,
                        "relativeStoppingTolerance");
            }
            ...
            if (childAlgorithm != null)
            {
                ...
                if (relativeStoppingTolerance > 0.0)
                {
                    ...
                    if (res != NloptResult.SUCCESS)
                        throw new ArgumentException("Unable to set secondary tolerance. Result: " + res,
                            "relativeStoppingTolerance");
                }
                ...
                if (ret != NloptResult.SUCCESS)
                    throw new ArgumentException("Unable to associate the child optimizer. Result: " + ret,
                        "childAlgorithm");
            }
        }
        ...
        {
            get { return nlopt_get_algorithm(_opt); }
        }
        ...
        {
            get { return nlopt_get_dimension(_opt); }
        }
        ...
        protected void CheckInequalityConstraintAvailability()
        {
            ...
            switch (algorithm)
            {
                ...
                default:
                    throw new ArgumentException("Algorithm " + algorithm.ToString() +
                                                " does not support inequality constraint.");
            }
        }
        ...
        protected void CheckEqualityConstraintAvailability()
        {
            ...
            switch (algorithm)
            {
                ...
                default:
                    throw new ArgumentException("Algorithm " + algorithm.ToString() +
                                                " does not support equality constraint.");
            }
        }
        ...
        {
            CheckEqualityConstraintAvailability();
            nlopt_mfunc mfunc = (m, results, n, values, gradient, data) =>
                Evaluate((int) n, values, (int) m, results, constraints);
        ...
        public void AddEqualZeroConstraints(Action<double[], double[], double[]> constraints, double[] tolerances)
        {
            CheckEqualityConstraintAvailability();
            nlopt_mfunc mfunc = (m, results, n, values, gradient, data) =>
                Evaluate((int) n, values, gradient, (int) m, results, constraints);
            _mfuncCache.Add(constraints, mfunc);
        ...
        public void SetRelativeToleranceOnOptimizationParameter(double tol)
        {
            ...
            if (res != NloptResult.SUCCESS)
                throw new ArgumentException("Unable to set the relative tolerance on optimization parameter. Result: " +
                                            res);
        }
        ...
        public void SetAbsoluteToleranceOnOptimizationParameter(double tol)
        {
            ...
            if (res != NloptResult.SUCCESS)
                throw new ArgumentException("Unable to set the absolute tolerance on optimization parameter. Result: " +
                                            res);
        }
...
#else
        private void Evaluate(int n, double[] values, double[] gradient, int m, double[] results,
            Action<double[], double[], double[]> action)
        {
            action.Invoke(values, gradient, results);
        }
#endif
